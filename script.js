document.addEventListener('DOMContentLoaded', function() {
    ...
    // Transport selection elements
    const transportCard = document.getElementById('transport-card');
    const transportButtons = document.querySelectorAll('.transport-btn');
    const selectedTransportMessage = document.getElementById('selected-transport-message');
    const transportCelebration = document.getElementById('transport-celebration');
    ...
    // Choose location button handler - now goes to transport selection first
    if (chooseLocationBtn) {
        chooseLocationBtn.addEventListener('click', function() {
            // Transition to transport selection first
            ...
            setTimeout(() => {
                ...
                setTimeout(() => {
                    ...
                    transportCard.style.opacity = '1';
                    transportCard.style.transform = 'scale(1)';
                    
                    // Create some hearts in the transport card
                    for (let i = 0; i < 15; i++) {
                        setTimeout(() => {
                            const heart = document.createElement('div');
                            heart.classList.add('heart');
                            ...
                            transportCelebration.appendChild(heart);
    ...
    // Transport selection with single choice
    transportButtons.forEach(button => {
        button.addEventListener('click', function() {
            // Remove selection from all transport buttons
            ...
            selectedTransportMessage.classList.remove('hidden');
            selectedTransportMessage.classList.add('show');
            confirmTransportBtn.style.display = 'inline-block';
        });
    });
    
    // Handle confirm transport button click
    if (confirmTransportBtn) {
        confirmTransportBtn.addEventListener('click', function() {
            if (selectedTransport) {
                // Store the selected transport
                localStorage.setItem('selectedTransport', selectedTransport);
                ...
                // After 1.2 seconds, proceed to location selection
                setTimeout(() => {
                    // Transition to location selection
                    transportCard.style.transform = 'scale(0.8)';
                    transportCard.style.opacity = '0';
    ...
    if (confirmDrinkBtn) {
        confirmDrinkBtn.addEventListener('click', function(e) {
            ...
            if (selectedDrinks.length > 0) {
                ...
                // Also send all current selections to backend
                const selectionsPayload = collectUserSelections();
                sendSelectionsToServer(selectionsPayload);
    ...
    // Handle No button click - show reason modal instead of converting to Yes
    ...
    // Collect all user selections from localStorage
    function collectUserSelections() {
        ...
        try {
            ...
            const loc = localStorage.getItem('selectedLocations');
            const dates = localStorage.getItem('dateOptions');
            const fd = localStorage.getItem('selectedFoods');
            const dr = localStorage.getItem('selectedDrinks');
            locations = loc ? JSON.parse(loc) : [];
            dateOptions = dates ? JSON.parse(dates) : [];
            foods = fd ? JSON.parse(fd) : [];
            drinks = dr ? JSON.parse(dr) : [];
        } catch (e) {
            console.warn('Could not parse selections from localStorage:', e);
        }
    ...
    // Send user selections to backend server
    function sendSelectionsToServer(selections) {
        const payload = {
            timestamp: new Date().toISOString(),
            userResponses: {
                transport: selections.transport,
                locations: selections.locations,
                dateOptions: selections.dateOptions,
                foods: selections.foods,
                drinks: selections.drinks,
                userAgent: selections.userAgent
            }
        };
        ...
        .then(data => {
            console.log('✅ Selections sent to server:', data);
        })
        .catch(error => {
            console.log('⚠️ Could not send selections to server:', error);
        });
    }
    ...
    function updateDrinkSelectionStatus() {
        ...
        if (selectedDrinks.length === 0) {
            statusContainer.classList.remove('active');
            statusContainer.innerHTML = '<p>Chọn sở thích đồ uống của bạn.</p>';
        } else {
            statusContainer.classList.add('active');
    ...
    const closeReasonModal = document.getElementById('close-reason-modal');
    ...
    reasonButtons.forEach(button => {
        button.addEventListener('click', function() {
            ...
            // Show custom reason container if "other" is selected
            if (reason === 'other') {
                customReasonContainer.classList.remove('hidden');
                submitReason.disabled = true; // Disable until text is entered
            } else {
                customReasonContainer.classList.add('hidden');
    ...
    if (customReasonTextarea) {
        customReasonTextarea.addEventListener('input', function() {
            customReason = this.value.trim();
            submitReason.disabled = !customReason;
        });
    }
    ...
    if (cancelReason) {
        cancelReason.addEventListener('click', hideReasonModal);
    }
    ...
    if (submitReason) {
        submitReason.addEventListener('click', function(e) {
            ...
            if (selectedReason === 'other' && !customReason) {
                ...
                customReasonTextarea.classList.add('shake');
                setTimeout(() => {
                    customReasonTextarea.style.borderColor = '';
                    customReasonTextarea.classList.remove('shake');
                }, 1000);
                return;
            }
            ...
            saveRejectionReason(finalReason);
    ...
    // Save rejection reason to localStorage
    function saveRejectionReason(reason) {
        const rejectionData = {
            reason: reason,
            timestamp: new Date().toISOString(),
            userAgent: navigator.userAgent
        ...
        localStorage.setItem('rejectionReason', JSON.stringify(rejectionData));
        
        // Also send to server if available (optional)
        sendRejectionDataToServer(rejectionData);
    }
    ...
    function showThankYouMessage() {
        ...
        thankYouDiv.innerHTML = `
            <div class="thank-you-content">
                ...
                <p>Tôi hiểu và tôn trọng quyết định của bạn.</p>
                <p>Hy vọng chúng ta có thể kết nối trong tương lai!</p>
            </div>
        `;
        ...
        setTimeout(() => {
            ...
            thankYouDiv.style.transform = 'translate(-50%, -50%) scale(1)';
        }, 10);
        ...
        setTimeout(() => {
            ...
            thankYouDiv.style.transform = 'translate(-50%, -50%) scale(0.8)';
            setTimeout(() => {
                thankYouDiv.remove();
            }, 300);
        }, 4000);
    ...
    // Send rejection data to server
    function sendRejectionDataToServer(rejectionData) {
        const serverData = {
        ...
        .then(response => response.json())
        .then(data => {
            console.log('✅ Rejection data sent to server:', data);
        })
        .catch(error => {
            console.log('⚠️ Could not send rejection data to server:', error);
            // This is not critical, data is still saved in localStorage
        });
    }
